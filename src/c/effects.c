...
static void set_pixel(BitmapInfo bitmap_info, int y, int x, uint8_t color)
{
  ...
  if (bitmap_info.bitmap_format == GBitmapFormat1Bit || bitmap_info.bitmap_format == GBitmapFormat1BitPalette)
  { // for 1 bit bitmap on Aplite  --- verify if it needs to be different
  ...
  { // othersise (assuming GBitmapFormat8Bit) going byte-wise
...
static uint8_t get_pixel(BitmapInfo bitmap_info, int y, int x)
{
  ...
  if (bitmap_info.bitmap_format == GBitmapFormat1Bit || bitmap_info.bitmap_format == GBitmapFormat1BitPalette)
  { // for 1 bit bitmap on Aplite - shifting right to get bit
    ...
    if (longLen > 0)
    {
      ...
      for (int j = 0x80 + (x << 8); y <= longLen; ++y)
      {
        ...
        if (temp_y >= bounds.origin.y && temp_y < bounds.size.h && temp_x >= bounds.origin.x && temp_x < bounds.size.w)
        {
          ...
          if (temp_pixel != skip_color && temp_pixel != draw_color)
            set_pixel(bitmap_info, temp_y, temp_x, draw_color);
#else
          if (((visited[temp_y * 20 + temp_x / 8] >> (temp_x % 8)) & 1) != 1)
          { // for Aplite first check if pixel isn't already marked as set in user-defined array
            if (temp_pixel != skip_color)
              set_pixel(bitmap_info, temp_y, temp_x, draw_color);                                                // if pixel isn't of original color - set it
...
void effect_mask(GContext *ctx, GRect position, void *param)
{
  ...
  else if (mask->bitmap_mask)
  { // othersise - bitmap mask is used - draw bimap
  ...
  for (int y = 0; y < position.size.h; y++)
    for (int x = 0; x < position.size.w; x++)
    {
      ...
      if (gcolor_contains(mask->mask_colors, temp_pixel))
      { // if array of mask colors matches current screen pixel color:
